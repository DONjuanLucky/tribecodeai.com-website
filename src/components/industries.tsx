...
export function Industries() {
  return (
    <section id="industries" className="py-32 relative overflow-hidden bg-[var(--bg-subtle)]" aria-labelledby="industries-heading">
      ...
      <div className="relative">
        ...
        <div className="flex animate-marquee w-max gap-6">
          {marqueeItems.map((industry, i) => (
            <div
              key={`${industry.name}-${i}`}
              className="group flex-shrink-0 w-64 p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border)] hover:border-[var(--accent)]/30 transition-colors duration-300 cursor-default"
            >
              ...
              <h3 className="text-lg font-display font-bold text-[var(--text-primary)] mb-1">{industry.name}</h3>
